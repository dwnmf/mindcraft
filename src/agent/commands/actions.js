...
export const actionsList = [
    ...
    {
        name: '!buildHouse',
        description: 'Builds a simple house.  Provide details like size, materials, and features.',
        params: {
            'details': { type: 'string', description: 'Details about the house (e.g., "5x5 wooden house with a glass roof").' }
        },
        perform: async function (agent, details) {
            if (!settings.allow_insecure_coding)
                return 'buildHouse not allowed! Code writing is disabled in settings. Notify the user.';

            // Chain-of-Thought Prompting for House Building
            const buildPrompt = `You are tasked with generating code to build a house in Minecraft based on the following details: "${details}".\n\nThink step by step about the construction process. Consider:\n\n1. **Materials:** What materials should be used for the walls, floor, roof, and other parts?\n2. **Dimensions:** What are the desired dimensions of the house (width, length, height)?\n3. **Features:**  Should the house include features like windows, doors, a crafting table, a bed, torches, etc.?\n4. **Location:** Where should the house be built (current position or find a suitable spot)?\n5. **Code Structure:**  Generate well-structured JavaScript code using the provided 'skills' and 'world' functions from the Minecraft library. Ensure all asynchronous operations use 'await'.\n\nNow, provide your step-by-step thought process and then the final generated code within a JavaScript code block.  Include helpful comments in your code.\n\nExample Thought Process and Code:\nUser Request: "small wooden house with a door"\nThought Process:\n1. Materials: Oak planks for walls, floor, and roof. Oak door for entrance.\n2. Dimensions: 5x5x3.\n3. Features: A single oak door.\n4. Location: Current position.\n5. Code:\n\`\`\`javascript\n// Building a 5x5x3 oak plank house with a door at the current position.\nconst pos = world.getPosition(bot);\nconst width = 5;\nconst depth = 5;\nconst height = 3;\nfor (let y = 0; y < height; y++) {\n  for (let x = 0; x < width; x++) {\n    for (let z = 0; z < depth; z++) {\n      if (y === 0 || y === height - 1 || x === 0 || x === width - 1 || z === 0 || z === depth - 1) {\n        await skills.placeBlock(bot, 'oak_planks', pos.x + x, pos.y + y, pos.z + z);\n      }\n    }\n  }\n}\n// Place door\nawait skills.placeBlock(bot, 'oak_door', pos.x + 2, pos.y + 1, pos.z, 'south'); // Example door placement\n\`\`\`\n\nNow generate the code for the requested house: "${details}"`

            this.agent.history.add("system", buildPrompt); // Add the build prompt to history for context


            return await agent.coder.generateCode(agent.history); // This triggers code generation
        }
    }
];
